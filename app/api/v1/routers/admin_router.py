...
@router.get("/", response_model=Sequence[Admin])
async def get_all_admins(session: AsyncSession = Depends(get_async_session), page: int = 1, page_size: int = 10):
    """
...
async def create_admin(admin_data: AdminCreate, session: AsyncSession = Depends(get_async_session)):
    """
    Creates a new admin in the database.
    """
    admin_repository = AdminRepository(session)

    admin = await admin_repository.create_admin(admin_data)
    return admin
